...
@test "confirm policy attached to DPSSimpleServiceAccountGroup" {
  run bash -c "aws iam list-group-policies --group-name DPSSimpleServiceAccountGroup"
  [[ "${output}" =~ "AssumeDPSSimpleServiceAccountRolePolicy" ]]
}

@test "confirm policy attached to DPSTeamMemberGroup" {
  run bash -c "aws iam list-attached-group-policies --group-name DPSTeamMemberGroup"
  [[ "${output}" =~ "ReadOnlyAccess" ]]
}

@test "evaluate policy attached to AssumeDPSSimpleServiceAccountRolePolicy" {
  run bash -c "aws iam get-group-policy --policy-name AssumeDPSSimpleServiceAccountRolePolicy --group-name DPSSimpleServiceAccountGroup"
