...
terraform output DPSNonprodServiceAccount_aws_access_key_id | sed 's/"//g' | opw write aws-dps-2 DPSNonprodServiceAccount-aws-access-key-id -

# write the prod service account credentials to secret store
terraform output DPSProdServiceAccount_encrypted_aws_secret_access_key | base64 -d | gpg -dq --passphrase ${GPG_KEY_PASSPHRASE} |  opw write aws-dps-1 DPSProdServiceAccount-aws-secret-access-key -
terraform output DPSProdServiceAccount_aws_access_key_id | sed 's/"//g' | opw write aws-dps-1 DPSProdServiceAccount-aws-access-key-id -
