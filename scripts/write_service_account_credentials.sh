...
op item create --category "API Credential" --title "twelve-aws" --vault cohorts DPSNonprodServiceAccount-aws-secret-access-key=$(terraform output DPSNonprodServiceAccount_encrypted_aws_secret_access_key | sed 's/"//g' | base64 -d | gpg -dq --passphrase ${GPG_KEY_PASSPHRASE})
op item create --category "API Credential" --title "twelve-aws" --vault cohorts DPSNonprodServiceAccount-aws-access-key-id=$(terraform output DPSNonprodServiceAccount_aws_access_key_id | tr -d \\n | sed 's/"//g')
